...
namespace ref
{
    ...
    bool is_phone_fit(std::string phone, std::string req)
    {
        ...
        std::copy_if(phone.begin(), phone.end(), std::back_inserter(raw_phone), isdigit);

        std::string raw_request;
        std::copy_if(req.begin(), req.end(), std::back_inserter(raw_request), isdigit);
